...
export async function generateILPAPDF(config: ILPAReportConfig): Promise<Buffer> {
  const pdf = new jsPDF({
  ...
  pdf.text(`Generated by Polibit`, pdf.internal.pageSize.getWidth() / 2, 50, { align: 'center' })
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    pdf.setFont('helvetica', 'normal')
    pdf.text(
      `Generated on ${new Date().toLocaleDateString()} | Polibit`,
