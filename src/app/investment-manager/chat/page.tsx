...
export default function InvestmentManagerChatPage() {
        ...
        if (result.success && result.data) {
          const apiConversations: Conversation[] = result.data.map((conv: any) => {
            ...
            if (conv.type === 'direct') {
              ...
              console.log('[Chat] Conversation participants:', conv.participants, 'currentUserId:', currentUser?.id)
              if (conv.participants && conv.participants.length > 0) {
                const otherParticipant = conv.participants.find((p: any) => {
                  const pId = p.userId || p.id || p.user_id
                  console.log('[Chat] Checking participant:', p, 'pId:', pId)
  ...
  const loadInvestorUsers = async () => {
    ...
    try {
      // Fetch users with investor role (role 3)
      const response = await fetch(getApiUrl(API_CONFIG.endpoints.getUsersByRole('3')), {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
        },
      ...
      if (response.ok) {
        const result = await response.json()
        if (result.success && result.data) {
          setInvestorUsers(result.data)
        }
      }
    } catch (error) {
      console.error('Error loading investor users:', error)
    } finally {
      setLoadingUsers(false)
    }
  }

  const createConversation = async (user: InvestorUser) => {
    const token = getAuthToken()
    if (!token) {
            ...
            <Dialog open={showNewChatDialog} onOpenChange={setShowNewChatDialog}>
              ...
              <DialogContent>
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Select an investor to start chatting with
                  </DialogDescription>
                </DialogHeader>
                <div className="space-y-2 max-h-96 overflow-y-auto">
                  {loadingUsers ? (
                    ...
                    <div className="text-center py-8 text-muted-foreground">
                      ...
                      <p className="text-sm">No investors available</p>
                    </div>
                  ) : (
          ...
          {filteredConversations.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 text-center px-4">
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Click "New" to start chatting with investors
              </p>
            </div>
          ) : (
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mb-1">
                        <p className="text-xs text-muted-foreground">
                          {conversation.type === 'structure' ? conversation.structureType : getRoleName(conversation.participantRole || 3)}
                        </p>
        ...
        {!selectedConversation ? (
          <div className="flex-1 flex flex-col items-center justify-center text-center p-8">
            ...
            <p className="text-muted-foreground max-w-md">
              Choose a conversation from the left or start a new chat with an investor
            </p>
          </div>
        ) : (
          <>
