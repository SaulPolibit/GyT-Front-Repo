...
### 7. Get Investor Commitments

**Endpoint:** `GET /api/investors/:id/commitments`

**Access:** Private (Root/Admin/Staff can access any investor, Customers can only access their own)

**Authorization Rules:**
- Role 0 (Root): Can access any investor's commitments
- Role 1 (Admin): Can access any investor's commitments
- Role 2 (Staff): Can access any investor's commitments
- Role 3 (Customer): Can only access their own commitments (matched by email)

**Description:**
Returns detailed commitment data for an investor including total commitments, called/uncalled capital, and a list of active fund structures they're invested in.
...
```json
{
  ...
  "data": {
    ...
    "calledCapital": 1500000,
    "uncalledCapital": 1000000,
    "activeFunds": 3,
    "structures": [
      {
        "id": "structure-uuid-1",
        ...
        "type": "Fund",
        "commitment": 1000000,
        "calledCapital": 600000,
        "uncalledCapital": 400000,
      ...
      {
        "id": "structure-uuid-2",
        "name": "Real Estate SPV - Miami",
        "type": "SA/LLC",
        "commitment": 500000,
        "calledCapital": 300000,
        "uncalledCapital": 200000,
...
{
  "success": false,
  "error": "Customers can only access their own investor data"
}
```
...
- Data comes from the `fundOwnerships` JSONB array in the `investors` table
- Total Commitment = Sum of all `commitment` values from fundOwnerships
- Called Capital = Sum of all `calledCapital` values from fundOwnerships
- Uncalled Capital = Sum of all `uncalledCapital` values from fundOwnerships
- Active Funds = Count of fundOwnerships entries where the related structure has status = 'Active'
- For role 3 users, verify the investor's email matches the authenticated user's email before returning data
- Structures list should only include active structures (filter by structure status)
- Sort structures by `investedDate` descending (most recent first)
